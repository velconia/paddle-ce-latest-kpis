...
PADDLE_BUILD_PATH=$PADDLE_PATH/build52/fix_hung_up
...
export PADDLE_DIST_CE_BASE_DIR="`basename $CURRENT_FILE_DIR`"
...
PADDLE_LOCAL_DOCKER_HUB_TAG=minqiyang/ce_dist_transformer:${PADDLE_COMMIT}
...
#PADDLE_CODE_PATH=paddle_dist_ce_transformer_${PADDLE_COMMIT}
...
rm -rf $LOCAL_IMAGE_DIR/*.whl && cp $PADDLE_BUILD_PATH/python/dist/*.whl $LOCAL_IMAGE_DIR && cd $CURRENT_FILE_DIR
...
--start-cmd "GLOG_logtostderr=1 GLOG_v=4 python -u train.py --src_vocab_fpath thirdparty/${TRANSFORMER_DIR}/vocab.bpe.32000 --trg_vocab_fpath thirdparty/${TRANSFORMER_DIR}/vocab.bpe.32000 --special_token '<s>' '<e>' '<unk>' --train_file_pattern thirdparty/${TRANSFORMER_DIR}/train.tok.clean.bpe.32000.en-de --token_delimiter ' ' --use_token_batch True --batch_size 3200 --sort_type pool --pool_size 200000 n_layer 6 n_head 16 d_model 1024 d_inner_hid 4096 dropout 0.3"
...
--k8s-wall-time ${PADDLE_CLOUD_JOB_WALL_TIME} \
--job-name ${PADDLE_CLOUD_JOB_NAME} \
--start-cmd "GLOG_logtostderr=1 GLOG_v=4 python train.py --src_vocab_fpath thirdparty/${TRANSFORMER_DIR}/vocab.bpe.32000 --trg_vocab_fpath thirdparty/${TRANSFORMER_DIR}/vocab.bpe.32000 --special_token '<s>' '<e>' '<unk>' --train_file_pattern thirdparty/${TRANSFORMER_DIR}/train.tok.clean.bpe.32000.en-de --token_delimiter ' ' --use_token_batch True --batch_size 3200 --sort_type pool --pool_size 200000 n_layer 6 n_head 16 d_model 1024 d_inner_hid 4096 dropout 0.3" \
--job-conf $PADDLE_CLOUD_HELPER_PATH/job_conf.py \
--files ${PADDLE_CLOUD_BENCHMARK_FILE} \
--k8s-not-local \
--k8s-trainers ${PADDLE_CLOUD_TRAINER_COUNT} \
